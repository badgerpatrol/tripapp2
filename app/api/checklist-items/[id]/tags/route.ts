...
/**
 * GET /api/checklist-items/[id]/tags - Get all tags for a checklist item
 *
...
export async function GET(
  ...
  try {
    ...
    const tags = await listTagsForEntity(TagEntityType.checklist_item, checklistItemId);

    return NextResponse.json({ success: true, tags }, { status: 200 });
  } catch (error) {
    console.error("Error fetching checklist item tags:", error);
...
/**
 * POST /api/checklist-items/[id]/tags - Link a tag to a checklist item
 *
 * Authorization: User must be a member of the trip that owns this checklist item
 * Body: { tagId: string }
 */
export async function POST(
  ...
  try {
    ...
    console.error("Error linking tag to checklist item:", error);
